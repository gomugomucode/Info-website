...
```
✅ Use nouns, not verbs in URLs  → /users not /get Users
...
## JSON Structure
**JSON (Java Script Object Notation)** is the universal data format for web APIs.
### Data Types
...
### Realistic API test vectors Example
...
### Working with JSON in Java Script
...
Authorization: Basic d XNlcjpw YXNzd29y ZA==
...
ey Jhb Gci Oi JIUz I1Ni Is In R5c CI6Ikp XVCJ9   ← Header (Base64)
.ey J1c2Vy SWQi Oj Es In Jvb GUi Oi Jh ZG1pbi J9  ← Payload (Base64)
. Sfl Kxw RJSMe KKF2QT4fwp Me Jf36POk6y JV    ← Signature (HMAC)
...
3. Client stores JWT (local Storage or http Only cookie)
...
// Send JWT with fetch
const token = local Storage.get Item('token');
fetch('/api/protected', {
...
✅ Store in http Only cookies (not local Storage) to prevent XSS
✅ Use HTTPS always
...
3. Server sends session ID in a cookie: Set-Cookie: session Id=abc123
...
Set-Cookie: user_pref=dark_mode; Path=/; Http Only; Secure; Same Site=Strict

# Browser sends cookie automatically
Cookie: session Id=abc123; user_pref=dark_mode
...
| Attribute   | Purpose                                           |
| `Http Only`  | JS cannot access — prevents XSS token theft       |
| `Secure`    | Cookie only sent over HTTPS                       |
| `Same Site`  | Controls cross-site sending (Strict/Lax/None)     |
...
| Storage Location   | Server (DB/memory)      | Client (cookie/local Storage) |
| Stateful?          | ✅ Yes                  | ❌ No (stateless)        |
| Scalability        | ❌ Harder (sticky sessions) | ✅ Easier (no server state) |
| Invalidation       | ✅ Easy (delete session) | ❌ Hard (wait for expiry) |
| Data in Token      | Only session ID         | Full user data in test vectors |
| Security Surface   | Session hijacking       | XSS (if in local Storage) |
| Best For           | Traditional web apps    | APIs, SPAs, Microservices |
### Security Best Practices

```
✅ Always use HTTPS
✅ Set Http Only + Secure flags on auth cookies
✅ Use Same Site=Strict or Lax to prevent CSRF
...
Set-Cookie: session Id=abc; Http Only; Secure
